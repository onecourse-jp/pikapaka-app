...
export default class ScrollPicker extends Component {
  ...
  componentDidMount() {
    if (this.state.selectedIndex) {
      console.log('this.props.selectedIndex', this.state.selectedIndex);
      setTimeout(() => {
        this.scrollToIndex(
  ...
  componentWillUnmount() {
    ...
    console.log('componentWillUnmount', this.state.selectedIndex);
  }
  ...
  render() {
    ...
    let highlightStyle = {
      position: 'absolute',
      //   backgroundColor: 'red',
      top: (this.state.windowHeight - this.itemHeight) / 2,
    ...
    // console.log('highlightStyle',wrapperStyle);
    return (
      <View style={wrapperStyle}>
        ...
        <ScrollView
          onLayout={(e) => {
            ...
            this.setState({ windowHeight: e.nativeEvent.layout.height });
          }}
          ref={(sview) => {
            this.sview = sview;
          }}
  ...
  _renderPlaceHolder() {
    let h = (this.state.windowHeight - this.itemHeight) / 2;
