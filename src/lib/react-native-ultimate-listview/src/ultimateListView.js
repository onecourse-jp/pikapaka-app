...
export default class UltimateListView extends Component {
  ...
  postRefresh = (rows = [], pageLimit) => {
    if (this.mounted) {
      ...
      if (rows.length < pageLimit || (pageLimit == -1 && rows.length == 0)) {
  ...
  paginationAllLoadedView = () => {
    if (this.props.pagination && this.state.dataSource.length > 9) {
  ...
  renderHeader = () => {
    if (this.props.header && this.getRows().length > 0) {
  ...
  render() {
    ...
    var renderHeader = {
      stickyHeaderIndices:
        this.state.dataSource.length > 0 ? this.props.stickyHeaderIndices : [],
    };
    if (this.props.header) {
      renderHeader['ListHeaderComponent'] = this.renderHeader;
    }
    if (!this.props.stickyHeaderIndices) {
      renderHeader['renderScrollComponent'] = this.renderScrollComponent;
    }
