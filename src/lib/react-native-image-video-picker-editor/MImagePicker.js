...
export default class MImagePicker extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      cropperParams: [],
      loading: false,
      showCamera: props.showCamera ?? false,
      mute: false,
      singleSelect: false,
    };
  }
  ...
  selectImage = (index) => {
    ...
    if (newSelected.indexOf(index) === -1) {
      ...
      if (
        this.state.photos[newSelected[deleteIndex]].node.image.filename ==
        this.state.defaultImage.filename
      ) {
  ...
  setDefualt = async () => {
    ...
    console.log('assetType2222', assetType);
    let params = {
      first: 1,
      assetType: assetType,
