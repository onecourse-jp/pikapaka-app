...
export const ImageCropper = async (params) => {
  ...
  var percentCropperAreaW = getPercentDiffNumberFromNumber(
    wScale,
    fittedSize.width,
  );
  var percentRestW = 100 - percentCropperAreaW;
  var hiddenAreaW = getPercentFromNumber(percentRestW, fittedSize.width);
  var percentCropperAreaH = getPercentDiffNumberFromNumber(
    hScale,
    fittedSize.height,
  );
  ...
  offset.x = x <= 0 ? 0 : x;
  offset.y = y <= 0 ? 0 : y;
  var srcPercentCropperAreaW = getPercentDiffNumberFromNumber(
    offset.x,
    fittedSize.width,
  );
  var srcPercentCropperAreaH = getPercentDiffNumberFromNumber(
    offset.y,
    fittedSize.height,
  );
...
export const HandleCrop = async (param) => {
  const { selected, photos, cropperParams, cropSize, HEADER_MAX_HEIGHT } =
    param;
      ...
      selected.map(async (t) => {
        ...
        if (image.playableDuration > 0) {
          try {
            ...
            result.push({ type: 'video', assest: { uri: vr } });
          } catch (error) {
            console.log(error);
          }
        } else {
          const crpIndex = cropperParams.findIndex(
          ...
          result.push({ type: 'photo', assest: { uri: eresult } });
        }
      }),
    );
    return result;
